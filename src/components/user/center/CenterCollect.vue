...
import { GetCenterCollect, PostBlogLikes, DeleteBlog } from "@/api/user/Center";
...
const 跳转到文章详情页 = (item) => {router.push(`/home/blogdetail/${item.blog_id}`)}

//初始化数据,获取文章数据的基础数据
const change = async () => {
    const res = await GetCenterCollect(props.user_id)
    文章数据.value = res.data
}
onMounted(() => { change() })
</script>
