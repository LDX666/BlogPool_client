<template>
    <div class="box" v-if="是否成功发布">
        ...
        <el-form :model="对话框数据" :rules="rules" label-width="auto" ref="校验结果" style="margin: 0px auto;width: 820px;">
            ...
            <el-form-item>
                <el-button type="primary" @click="确认()">确认</el-button>
            </el-form-item>
        </el-form>
    </div>
...
import { PostBlog } from "@/api/user/Blog";
...
//确认按钮的功能
const 确认 = async () => {
    console.log('点击确认按钮时的数据:', 对话框数据.value);
    ElMessageBox.confirm('是否确认', 'Warning', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning', })
        .then(async () => {
            try {
                ...
                let data = { user_id, blog_img, title, type_id, introduce, blog_content }
                const res = await PostBlog(data)
                if (res.code === 200) {
                    ElMessage({ message: '添加文章成功', type: 'success', })
                    是否成功发布.value = false
                } else {
                    ElMessage.error('添加文章失败,请再试一次')
                }
            } catch (error) {
