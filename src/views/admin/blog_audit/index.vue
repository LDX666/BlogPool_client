<template>
    <div class="admin_router">
        ...
        <el-tab-pane label="系列管理" name="0">
            <AdminBlogAuditHome @emit_查看按钮="打开标签页" />
        </el-tab-pane>
        <el-tab-pane v-for="item in 标签组" :key="item.name" :label="item.title" :name="item.name" style="background-color: #f2f3f5;">
...
<script setup>
import { ref } from 'vue'
import AdminBlogAuditHome from "@/components/admin/blog_audit/AdminBlogAuditHome.vue"
...
const 打开标签页 = async (data)=>{//这个data是子组件事件触发的时候传过来的值
    ...
    标签组.value.push({
        ...
        props:data   //将props作为值直接传给这个数组,因为数组直接被v-for解析了,这样可以传给里面的子组件,
